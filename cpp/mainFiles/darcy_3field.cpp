...
// #define DARCY_EXAMPLE_TWO_BC_ANNULUS_PLUS_INTERFACE
  ...
  int main(int argc, char** argv ) {
    ...
    for(int i=0; i<iters; ++i) {
        ...
        // //  innerProduct(uPenParam*pow(h,1)*jump(u), jump(v))
        // // +innerProduct(uPenParam*pow(h,3)*jump(grad(u)*n), jump(grad(v)*n))
        // // +innerProduct(uPenParam*pow(h,5)*jump(grad2un), jump(grad2un))
        // // +innerProduct(pPenParam*pow(h,1)*jump(p), jump(q))
        // // +innerProduct(pPenParam*pow(h,3)*jump(grad(p)), jump(grad(q)))

        // +innerProduct(uPenParam*pow(h,1)*jump(u), jump(v)) 
        // +innerProduct(uPenParam*pow(h,3)*jump(grad(u)*n), jump(grad(v)*n))
        // +innerProduct(uPenParam*pow(h,5)*jump(grad2un), jump(grad2un))
        // -innerProduct(pPenParam*pow(h,1)*jump(p), jump(div(v)))
        // +innerProduct(pPenParam*pow(h,1)*jump(div(u)), jump(q))
        // // -innerProduct(pPenParam*pow(h,3)*jump(grad(p)), jump(grad(div(v))))
        // // +innerProduct(pPenParam*pow(h,3)*jump(grad(div(v))) , jump(grad(q)))
        // -innerProduct(pPenParam*pow(h,3)*jump(grad(p)*n), jump(grad(div(v))*n))
        // +innerProduct(pPenParam*pow(h,3)*jump(grad(div(u))*n) , jump(grad(q)*n))
        // // -innerProduct(pPenParam*pow(h,5)*jump(grad2pn), jump(grad2divun))
        // // +innerProduct(pPenParam*pow(h,5)*jump(grad2divun), jump(grad2pn))
        // , Kh_i
        // , macro
        // );
        ...
        darcy.addFaceStabilization( // h^(2k)
        -innerProduct(itfPenParam*pow(h,-1)*jump(p_itf), jump(q_itf))
        -innerProduct(itfPenParam*pow(h,1)*jump(grad(p_itf)*n), jump(grad(q_itf)*n)) 
        -innerProduct(itfPenParam*pow(h,3)*jump(grad2pitfn), jump(grad2pitfn))
        , Kh_itf
        // , macro_itf
        );
        darcy.addBilinear(
        -innerProduct(itfPenParam*pow(h,0)*jump(p_itf), jump(q_itf))
        -innerProduct(itfPenParam*pow(h,2)*jump(grad(p_itf)*n), jump(grad(q_itf)*n)) 
        -innerProduct(itfPenParam*pow(h,4)*jump(grad2pitfn), jump(grad2pitfn))
        , interface
        );
  ...
  int main(int argc, char** argv ) {
    ...
    for(int i=0; i<iters; ++i) {
        ...
        // );
        darcy.BaseFEM::addBilinear(
          -innerProduct(1e-3*itfPenParam*pow(h,0)*grad(p_itf)*n, grad(q_itf)*n) // scaling like the interface stab!
        , Kh_itf
        );
